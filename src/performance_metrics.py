...
class TranspilerBenchmark:
    ...
    def _create_simple_noise_model(self) -> NoiseModel:
        ...
        readout_error = ReadoutError(
            [
                [1 - prob_meas1given0, prob_meas1given0],
    ...
    def _transpile_with_timing(
        self, circuit: QuantumCircuit, pass_manager: PassManager
    ...
    def benchmark_single_circuit(self, circuit: QuantumCircuit) -> Dict[str, Any]:
        ...
        try:
            ...
            transpiled_custom, custom_time = self._transpile_with_timing(
                circuit, custom_pm
            )
            ...
            print(
                f"Custom Transpiler: Depth={custom_metrics.depth}, "
        ...
        for level in self.optimization_levels:
            try:
                transpiled_qiskit, qiskit_time = self._transpile_with_qiskit(
                    circuit, level
                )
                ...
                circuit_results["qiskit"][f"level_{level}"]["transpiled_circuit"] = (
                    transpiled_qiskit
                )

                print(
                    f"Qiskit (Level {level}): Depth={qiskit_metrics.depth}, "
    ...
    def run_full_benchmark(
        self, circuits: Optional[List[QuantumCircuit]] = None
    ) -> Dict[str, Any]:
        """
        ...
        print(
            f"Starting benchmark with {len(circuits)} circuits on {self.backend_name}"
        )
    ...
    def generate_summary_table(self) -> pd.DataFrame:
        ...
        for circuit_name, data in self.results.items():
            ...
            for level in self.optimization_levels:
                ...
                if level_key in data["qiskit"] and not data["qiskit"][level_key].get(
                    "error_message"
                ):
                    row.update(
                        {
                            f"Qiskit_L{level}_Depth": data["qiskit"][level_key][
                                "depth"
                            ],
                            f"Qiskit_L{level}_Gates": data["qiskit"][level_key][
                                "num_gates"
                            ],
                            f"Qiskit_L{level}_CX": data["qiskit"][level_key]["num_cx"],
                            f"Qiskit_L{level}_Time": data["qiskit"][level_key][
                                "transpilation_time"
                            ],
                        }
                    )
    ...
    def print_summary(self):
        ...
        for circuit_name, data in self.results.items():
            ...
            orig = data["original"]
            print(
                f"Original:    Depth={orig['depth']:3d}, Gates={orig['num_gates']:3d}, CX={orig['num_cx']:3d}"
            )
            ...
            if "custom" in data and not data["custom"].get("error_message"):
                ...
                print(
                    f"Custom:      Depth={custom['depth']:3d} ({depth_improvement:+d}), "
            ...
            for level in self.optimization_levels:
                ...
                if level_key in data["qiskit"] and not data["qiskit"][level_key].get(
                    ...
                    print(
                        f"Qiskit L{level}:   Depth={qiskit['depth']:3d} ({depth_improvement:+d}), "
...
def create_performance_comparison(
    backend_name: str = "FakeTorino", max_qubits: int = 5, save_results: bool = True
) -> TranspilerBenchmark:
    """
...
if __name__ == "__main__":
    ...
    benchmark = create_performance_comparison(
        backend_name="FakeTorino", max_qubits=5, save_results=True
    )
