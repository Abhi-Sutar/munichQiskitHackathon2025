...
class DecoherenceWatchdogPass(TransformationPass):
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        if 'layout' not in self.property_set or 'node_start_time' not in self.property_set:
            print("[WatchdogPass] Layout and scheduling info not available. Skipping modification.")
            return dag
        ...
        worst_spot = { "cost": -1, "qubit": None, "idle_start_dt": None, "idle_end_dt": None }
                ...
                if idle_duration_sec > 1e-9: # Only consider non-trivial idle times
                    ...
                    if idling_cost > worst_spot["cost"]:
                        worst_spot = {
                            ...
                            "idle_start_dt": schedule[prev_node] + round(prev_duration_sec / dt),
        ...
        cr = ClassicalRegister(1, 'c_watchdog') # A dedicated classical bit for the herald
        ...
        dag.compose(gadget_qc, qubits=[data_qubit, ancilla_qubit], clbits=[dag.clbits[-1]])
        print("[WatchdogPass] Watchdog gadget successfully inserted.")
        return dag

# You'll build your custom PassManager using your custom passes
def get_custom_pass_manager(backend=None, pass_class=None, pass_kwargs=None, include_optimization=True):
    """
