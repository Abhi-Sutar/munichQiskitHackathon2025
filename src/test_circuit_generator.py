...
def get_efficient_su2_circuit(num_qubits: int, reps: int = 1, entanglement: str = 'linear') -> QuantumCircuit:
    """
...
def get_qv_circuit(num_qubits: int, depth: int = None, seed: int = None) -> QuantumCircuit:
    """
...
def generate_all_test_circuits(max_qubits_for_backend: int = None) -> list[QuantumCircuit]:
    """
    ...
    for n_q, depth, seed in random_configs:
        if max_qubits_for_backend is None or n_q <= max_qubits_for_backend:
            ...
            effective_depth = min(depth, 100) # Arbitrary limit for very deep random circuits on simulators
...
if __name__ == "__main__":
    ...
    for i, qc in enumerate(test_circs):
        print(f"  Circuit {i+1}: {qc.name} ({qc.num_qubits} qubits, Depth: {qc.depth()})")
