...
class DecoherenceWatchdog(TransformationPass):
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for period in idle_periods:
            vulnerability = self._get_idle_cost(
                period["duration_sec"], period["qubit_idx"]
            )
        ...
        print(
            f"[WatchdogPass] Found most vulnerable spot on qubit {target_idle_info['qubit_idx']} "
        ...
        watchdog_sequence_dag.apply_operation_back(
            CXGate(), qargs=[data_qubit_ref, ancilla_qubit_ref]
        )
        ...
        watchdog_sequence_dag.apply_operation_back(
            Delay(idle_duration_dt, "dt"), qargs=[data_qubit_ref]
        )
        ...
        watchdog_sequence_dag.apply_operation_back(HGate(), qargs=[ancilla_qubit_ref])
        watchdog_sequence_dag.apply_operation_back(
            Measure(), qargs=[ancilla_qubit_ref], cargs=[herald_bit_ref]
        )
        ...
        qubit_map = {
            data_qubit_ref: data_qubit_main,
            ancilla_qubit_ref: ancilla_qubit_main,
        }

        # Replace the single Delay node with our entire watchdog sequence
        dag.substitute_node_with_dag(
            target_node, watchdog_sequence_dag, wires=qubit_map
        )
...
def run_benchmark():
    ...
    try:
        ...
        print(
            f"   Successfully transpiled and deflated watchdog circuit with {watchdog_t_qc.num_qubits} qubits"
        )
    except Exception as e:
        print(f"   Transpilation failed: {e}")
        print(
            f"   Circuit has {watchdog_qc.num_qubits} qubits, backend supports {backend.num_qubits}"
        )
    ...
    print(
        f"{'5-Bit':<8} {'Data':<6} {'1)Ideal':<10} {'2)Std':<10} {'3)Raw':<10} {'4)PS':<10}"
    )
    print(
    ...
    for outcome in herald_0_states:
        ...
        print(
            f"{outcome:<8} {data_bits:<6} {ideal_p:<10.4f} {std_p:<10.4f} {raw_p:<10.4f} {ps_p:<10.4f}{ghz_marker}"
        )
    ...
    if herald_1_states:
        ...
        for outcome in herald_1_states:
            ...
            ps_p = ps_probs.get(outcome, 0.0)
            print(
                f"{outcome:<8} {data_bits:<6} {ideal_p:<10.4f} {std_p:<10.4f} {raw_p:<10.4f} {ps_p:<10.4f}"
            )
    ...
    print("1) Ideal: Noiseless simulation")
    print("2) Std: Noisy with Qiskit optimization_level=3")
    print("3) Raw: Noisy with watchdog transpiler (no post-selection)")
    print("4) PS: Noisy with watchdog transpiler (post-selected on herald='0')")
